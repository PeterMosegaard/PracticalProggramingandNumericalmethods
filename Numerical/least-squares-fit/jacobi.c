...
/*This function calculates the eigenvalues and stores them in the vector 
e and eigenvectors in the matrix V of A using cyclic sweeps*/
...
gsl_matrix_set_identity(V);
do{ changed=0; sweeps++; int p,q;
        for(p=0;p<n_val;p++)for(q=p+1;q<n;q++){
                double app=gsl_vector_get(e,p);
                double aqq=gsl_vector_get(e,q);
                double apq=gsl_matrix_get(A,p,q);
                double phi=0.5*atan2(2*apq,aqq-app);
